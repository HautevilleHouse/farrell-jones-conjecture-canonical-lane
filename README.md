...
Canonical Lane research workspace for a flagship problem in algebraic topology, K-theory, and geometric group theory:
proving persistence of the `K`- and `L`-theoretic assembly package through an admissible geometric-group closure architecture.

## Main Manuscript

- [paper/FARRELL_JONES_CONJECTURE_PREPRINT.md](paper/FARRELL_JONES_CONJECTURE_PREPRINT.md)
- [paper/CANONICAL_ROUTING_INDEX.md](paper/CANONICAL_ROUTING_INDEX.md)

## Structure

- `paper/`
  - `FARRELL_JONES_CONJECTURE_PREPRINT.md`
...
1. Theorem chain first: read `paper/FARRELL_JONES_CONJECTURE_PREPRINT.md`.
...
Current FJ runner policy:

- `repro/run_repro.sh` extracts, promotes, runs `scripts/fj_closure_guard.py`, updates `repro/repro_manifest.json`, and enforces `fully_extracted` release-gate mode.
...
3. artifact key consumed by `scripts/fj_closure_guard.py`.
...
- Manuscript target: [paper/FARRELL_JONES_CONJECTURE_PREPRINT.md](paper/FARRELL_JONES_CONJECTURE_PREPRINT.md)
