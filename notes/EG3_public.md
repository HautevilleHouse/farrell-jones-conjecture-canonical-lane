...
In-paper anchor: `paper/FARRELL_JONES_CONJECTURE_PREPRINT.md` (`FJ_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving persistence of the `K`- and `L`-theoretic assembly package through an admissible geometric-group closure architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`FJ_G3` closes when `kappa_compact` provides compactness/no-collapse control: normalized near-failure families are precompact and control windows do not collapse.
This is the first-failure capture contribution to `M_FJ`.
...
If near-failure extraction and no-Zeno spacing hold, then `FJ_G3` prevents hidden drift before rigidity is applied.
