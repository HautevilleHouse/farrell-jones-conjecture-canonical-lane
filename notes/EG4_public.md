...
In-paper anchor: `paper/FARRELL_JONES_CONJECTURE_PREPRINT.md` (`FJ_G4`, `FJ_G5`).

## Goal
Separate the rigidity job from the endpoint-transfer job for `proving persistence of the `K`- and `L`-theoretic assembly package through an admissible geometric-group closure architecture`.
...
- endpoint-transfer lock: `isomorphism_transfer`.
- coherence interface: `eps_coh` remains available to the bridge and final margin.

## Closure Criterion

`FJ_G4` closes when `rho_rigidity` excludes bad endpoint alternatives: bad nonisomorphic assembly countermodels are excluded.
`FJ_G5` closes when `isomorphism_transfer` transfers the surviving endpoint to the intended target class: rigid limit transfers to the assembly-isomorphism endpoint class.
Together they feed the strict margin `M_FJ`.
...
The surviving rigid representative is locked to the standard problem-side endpoint by `isomorphism_transfer`.

Payload: read this note together with `notes/IDENTIFICATION_BRIDGE.md`.

### Theorem EG4.3 (rigidity/transfer gate closure)
If bad limits are excluded and the endpoint lock is active, then `FJ_G4` and `FJ_G5` deliver the boundary object needed by the final margin.
...
- transfer gate: `FJ_G5`
- transfer artifact key: `isomorphism_transfer`
