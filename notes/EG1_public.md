...
In-paper anchor: `paper/FARRELL_JONES_CONJECTURE_PREPRINT.md` (`FJ_G1`).

## Goal
Make the projected response floor explicit as the protected-core gate for `proving persistence of the `K`- and `L`-theoretic assembly package through an admissible geometric-group closure architecture`.
...
`FJ_G1` closes when `kappa_assembly` satisfies the response-floor requirement: projected assembly response has a strict positive floor.
This is the protected-core contribution to the strict margin `M_FJ`.
...
Payload: check that all quantities used by `kappa_assembly` are defined on the projected sector named in the main preprint.
...
If Lemmas EG1.1-EG1.2 hold and the runtime artifact accepts `kappa_assembly`, then `FJ_G1` supplies the projected/protected-core input to `M_FJ`.
