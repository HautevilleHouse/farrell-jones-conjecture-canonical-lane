...
In-paper anchor: `paper/FARRELL_JONES_CONJECTURE_PREPRINT.md` (bridge and margin interface).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving persistence of the `K`- and `L`-theoretic assembly package through an admissible geometric-group closure architecture`.
...
- endpoint lock: `isomorphism_transfer`.
...
2. the endpoint-transfer lock `isomorphism_transfer` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_FJ` remains positive.
...
Payload: verify `isomorphism_transfer` against the constants registry and runtime certificate.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_FJ` is meaningful.

## Current Instantiation

- endpoint-transfer artifact key: `isomorphism_transfer`
